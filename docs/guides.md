...
Two things paddock does that are worth a walkthrough, because in both cases FreeToken has
...
2. `f` on one to fetch it into the local store under `~/.local/state/paddock/templates`.
...
There is no `ft serve` flag for the values themselves, so to choose them paddock changes what
...
original is kept beside it as `generation_config.json.paddock-original`, so `u` puts back
exactly what shipped.

Two things worth knowing. Greedy decoding ignores `top_k` and `top_p` entirely, so setting
either without a temperature does nothing — paddock says so before it writes. And a **GGUF**
