...
/// every single request the engine serves.
pub fn preflight_command(
    ft: &crate::ft::Freetoken,
    model_dir: &Path,
    jinja_path: &Path,
) -> Option<Vec<String>> {
    // Only the Python form can run an inline script; for a bare `ft` binary, find the
    // interpreter beside it in the same venv.
    let python = if ft.prefix.first().map(String::as_str) == Some("-m") {
        ft.program.clone()
    } else {
        let candidate = ft.program.parent()?.join("python");
        crate::ft::locate::is_executable(&candidate).then_some(candidate)?
    };
    Some(vec![
        python.display().to_string(),
        "-c".into(),
        PREFLIGHT_SCRIPT.into(),
